namespace MSharp.Framework.Services
{
    ...
    public class IpFilter
    {
        ...
        static FileInfo CountryIpsFile
        {
            get
            {
                return Path.Combine(Document.GetPhysicalFilesRoot(Document.AccessMode.Secure).FullName, "IPBlock\\dbip-country.csv").AsFile();
            }
        }
        ...
        public static void SetSpecificIpPolicy(Policy policy, params string[] ipAddresses)
        {
            foreach (var ip in ipAddresses)
            {
                ...
                else if (policy == Policy.Disallow)
                {
        ...
        static void EndWithMessage() => HttpContext.Current.Response.EndWith(BlockedAttemptResponse);
        ...
        static void LoadBlockedIpRanges()
        {
            ...
            lock (SyncLock)
                if (BlockedIpRanges == null)
                    DoLoadBlockedIpRanges();
        }

        static void DoLoadBlockedIpRanges()
        {
