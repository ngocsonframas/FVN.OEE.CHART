...
namespace System
{
    partial class MSharpExtensions
    {
        ...
        static Random RandomProvider;
        static Random GetRandomProvider() => RandomProvider ?? (RandomProvider = new Random(LocalTime.Now.TimeOfDay.Milliseconds));
        ...
        public static T PickRandom<T>(this IEnumerable<T> list)
        {
            if (list.Any())
            {
                var index = GetRandomProvider().Next(list.Count());
        ...
        public static IEnumerable<T> PickRandom<T>(this IEnumerable<T> list, int number)
        {
            ...
            var randomIndices = GetRandomProvider().PickNumbers(number, 0, items.Count - 1);
