...
namespace AE.Net.Mail.Imap
{
    public static class ModifiedUtf7Encoding
    {
        ...
        public static string Decode(string input)
        {
            ...
            {
                var indexOfMinus = result.IndexOf('-', indexOfAmpersand);
                if (indexOfMinus > 0)
                {
                    var substring = result.Substring(indexOfAmpersand + 1, indexOfMinus - indexOfAmpersand - 1);
                    var modifiedBase64 = "+" + substring.Replace(',', '/');
        ...
        static bool IsPrintableAscii(char c) => c >= '\x20' && c <= '\x7e';
    }
}