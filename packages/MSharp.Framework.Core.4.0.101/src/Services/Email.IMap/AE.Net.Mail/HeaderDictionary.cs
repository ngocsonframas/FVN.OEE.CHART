...
namespace AE.Net.Mail
{
    public class HeaderDictionary : SafeDictionary<string, HeaderValue>
    {
        ...
        public virtual string GetBoundary() => this["Content-Type"]["boundary"];
        ...
        public virtual void Add(string name, string value) => this[name] = new HeaderValue(value);
        ...
        public virtual MailAddress[] GetMailAddresses(string header)
        {
            ...
            while (mailAddressEndIndex < headerValue.Length)
            {
                ...
                if (mailAddressEndIndex == NOT_FOUND)
                {
        ...
        static int IndexOfExcludingQuoted(string str, char searchChar, int startIndex)
        {
            ...
            for (int i = startIndex; i < str.Length; i++)
            {
                if (!withinQuotes)
                {
                    ...
                    if (str[i] == '"') withinQuotes = true;
                }
                else if (str[i] == '"') withinQuotes = false;
            }
