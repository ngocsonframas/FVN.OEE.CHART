...
namespace AE.Net.Mail
{
    ...
    public class MailMessage : ObjectWHeaders
    {
        ...
        bool HeadersOnly; // set to true if only headers have been fetched. 
        ...
        public virtual void Load(Stream reader, bool headersOnly = false, int maxLength = 0, char? termChar = null)
        {
            ...
            if (!headersOnly)
            {
                ...
                if (!string.IsNullOrEmpty(boundary))
                {
                    ...
                    if (!string.IsNullOrEmpty(body)) SetBody(body);
        ...
        static string ParseMime(Stream reader, string boundary, ref int maxLength, ICollection<Attachment> attachments, Encoding encoding, char? termChar)
        {
        ...
        static Dictionary<string, int> FlagCache = System.Enum.GetValues(typeof(Flags)).Cast<Flags>().ToDictionary(x => x.ToString(), x => (int)x, StringComparer.OrdinalIgnoreCase);
        internal void SetFlags(string flags)
        {
            ...
            {
                if (FlagCache.TryGetValue(x.TrimStart('\\'), out var flag))
                    return flag;
                else return 0;
            }).Sum();
        }
